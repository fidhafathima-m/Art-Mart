...
const forgotPassValid = async(req, res) => {
    try {
        ...
        if(findUser) {
            ...
            res.render('admin-forgot-password', {message: 'Admin with this email does not exists.'})
        }
    ...
    try {
        ...
        if (password === confirmPassword) {
            ...
            await User.updateOne(
                {email: email},
                {password: passwordHashh}
            );
            res.redirect('/admin/login');
        } else {
            res.render('admin-reset-password', {message: 'Passwords doesn\'t match'})
        }
